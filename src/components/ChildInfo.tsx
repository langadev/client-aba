...
export default function ChildInfoPage() {
  ...
  const [child, setChild] = useState<ChildWithPsychologists | null>(null);
  ...
  useEffect(() => {
    const run = async () => {
      try {
        ...
        // 1) Criança + psicólogos vinculados (N:N) com retrocompatibilidade
        const childData = await getChildByIdWithPsychologists(Number(id));
        setChild(childData);

        const nameFromNn =
          childData.psychologistData?.name ||
          (childData.psychologists?.length
            ? childData.psychologists.map((p) => p.name).filter(Boolean).join(", ")
            : "");

        setPsychologistName(nameFromNn || "");
  ...
  const quickActions: QuickAction[] = [
    {
      ...
      subtitle: "Resumo de Progresso",
      icon: <Download className="w-5 h-5 text-blue-700" />,
      bg: "bg-blue-50",
      onClick: () => console.log("baixar relatório"),
    },
    {
      id: "schedule",
    ...
    },
    {
      ...
      subtitle: "Envie uma mensagem segura",
      icon: <MessageCircle className="w-5 h-5 text-purple-700" />,
      bg: "bg-purple-50",
      onClick: () => console.log("mensagem ao terapeuta"),
    },
    {
      id: "billing",
      title: "Ver Faturação",
      ...
      bg: "bg-orange-50",
      onClick: () => console.log("ver faturação"),
    },
    {
      id: "help",
      title: "Ajuda & Suporte",
      subtitle: "Obtenha assistência",
      icon: <HelpCircle className="w-5 h-5 text-gray-700" />,
      bg: "bg-gray-100",
      onClick: () => console.log("ajuda & suporte"),
    },
  ];
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      {/* TOPO: Progresso | Próxima Sessão | Atividade */}
      <div className="grid grid-cols-1 lg:grid-cols-3 gap-6 pb-6">
        ...
        <NextSessionCard next={nextSession} therapistName={psychologistName} />
      ...
      {/* Sessões Recentes + Ações Rápidas */}
